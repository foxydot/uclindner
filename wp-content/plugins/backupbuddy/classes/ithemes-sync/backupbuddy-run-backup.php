<?php
class Ithemes_Sync_Verb_Backupbuddy_Run_Backup {
	...
	public function run( $arguments ) {
		...
		if ( is_numeric( $profile ) ) {
			if ( isset( pb_backupbuddy::$options['profiles'][ $profile ] ) ) {
				...
				return array(
					'success' => '0',
					'status' => 'Error #2332904: Invalid profile ID `' . htmlentities( $profile ) . '`. Profile with this number was not found. Try deactivating then reactivating the plugin. If this fails please reset the plugin Settings back to Defaults from the Settings page.';
				);
			}
		} else {
			return array(
				'success' => '0',
				'status' => 'Error #85489548955. Invalid profile ID not numeric: `' . htmlentities( $profile ) . '`.';
			);
		}
		...
		if ( pb_backupbuddy::$classes['backup']->start_backup_process(
			...
			return array(
				'success' => '0',
				'status' => 'Error #435832: Backup failed. See BackupBuddy log for details.';
			);
		}
