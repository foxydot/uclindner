...
if ( '' != $configFile ) {
	...
	// Connect to database.
	global $wpdb;
	if ( false === ( $wpdb->dbh = @mysql_connect( $databaseSettings['host'], $databaseSettings['username'], $databaseSettings['password'] ) ) ) { // CONNECT failed.
		pb_backupbuddy::alert( 'Error #848383. Unable to connect to database with wp-config.php database settings.' );
		return;
	}
	if ( false === @mysql_select_db( $databaseSettings['name'] ) ) { // SELECT failed.
		pb_backupbuddy::alert( 'Error #3479302. Unable to select database with wp-config.php database settings.' );
		return;
	}
