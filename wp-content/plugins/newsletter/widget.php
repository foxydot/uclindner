    ...
    function widget($args, $instance) {
       ...
       // echo $before_widget;
        ...
        if (stripos($instance['text'], '<form') === false) {
            ...
            if ($options_profile['name_status'] == 2) $form .= '<p><input class="newsletter-firstname" type="text" name="nn" value="' . $options_profile['name'] . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/>';

            if ($options_profile['surname_status'] == 2) $form .= '<p><input style="background: none repeat scroll 0 0 #F0F0F0;     font-family: \'ralewayregular\';     height: 31px;     margin: 0 0 0 8px;     width: 120px;"  class="newsletter-lastname" type="text" name="ns" value="' . $options_profile['surname'] . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/ placeholder="e-mail">';

            $form .= '<input style="background: none repeat scroll 0 0 #F0F0F0;     font-family: \'ralewayregular\';     height: 31px;     margin: 0 0 0 8px;     width: 120px;" class="newsletter-email" type="email" required name="ne" value="' . $options_profile['email'] . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/ placeholder="e-mail">';

            if ($options_profile['sex_status'] == 2) {
                $form .= '<select name="nx" class="newsletter-sex">';
            ...
            for ($i = 1; $i <= NEWSLETTER_PROFILE_MAX; $i++) {
                ...
                if ($options_profile['profile_' . $i . '_type'] == 'text') {
                    $form .= '<input class="newsletter-profile newsletter-profile-' . $i . '" type="text" name="np' . $i . '" value="' . $options_profile['profile_' . $i] . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/>';
                }
                if ($options_profile['profile_' . $i . '_type'] == 'select') {
                    $form .= '' . $options_profile['profile_' . $i] . '<br /><select class="newsletter-profile newsletter-profile-' . $i . '" name="np' . $i . '">';
            ...
            if ($options_profile['privacy_status'] == 1) {
                if (!empty($options_profile['privacy_url'])) {
                    $form .= '<input type="checkbox" name="ny"/>&nbsp;<a target="_blank" href="' . $options_profile['privacy_url'] . '">' . $options_profile['privacy'] . '</a>';
                }
                else $form .= '<input type="checkbox" name="ny"/>&nbsp;' . $options_profile['privacy'] . '';
            }

            if (strpos($options_profile['subscribe'], 'http://') !== false) {
                $form .= '<input style="font-size:11px;color: #1E8E83;     cursor: pointer;     float: right;     font-family: \'ralewaysemibold\';margin: 9px 2px 0 0;" type="submit" value="Go"/>';
            } else {
                $form .= '<input style="font-size:11px;color: #1E8E83;     cursor: pointer;     float: right;     font-family: \'ralewaysemibold\';margin: 9px 2px 0 0;" type="submit" value="Go"/>';
            }
